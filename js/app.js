...
async function getProducts(){
    let data = await fetch('https://webshop.wm3.se/api/v1/shop/products.json?media_file=true')
    .then(checkStatus)
    .then(data => data.json())
    .then(createAndDisplayProducts)
    .catch(displayErrorMessage);

    // createAndDisplayProducts(data);
}
/**
...
async function searchProduct(searchTerm) {
    if (searchTerm === '') {
        ...
        let data = await fetch(`https://webshop.wm3.se/api/v1/shop/products/search.json?q=${searchTerm}&media_file=true`)
        .then(checkStatus)
        .then(data => data.json())
        .then(createAndDisplayProducts)
        .catch(displayErrorMessage);
...
window.addEventListener('scroll', () => {
    ...
    let offSet = window.scrollY - (carousel.clientHeight / 2);
    let alpha = offSet / 300;
